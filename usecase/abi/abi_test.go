...
func TestAbi(t *testing.T) {
	...
	t.Run("TestDecodeMessageBody", func(t *testing.T) {
		message := ""
		boc, err := bocUC.ParseMessage(&domain.ParamsOfParse{Boc: message})
		assert.Equal(t, nil, err)

		err = json.Unmarshal(boc.Parsed, &objmap)
		assert.Equal(t, nil, err)
	...
	t.Run("TestEncodeInternalMessageRun", func(t *testing.T) {
		...
		result, err := abiUC.EncodeInternalMessage(&domain.ParamsOfEncodeInternalMessage{Abi: helloAbi, Value: "1000000000", Address: address, CallSet: &domain.CallSet{FunctionName: "sayHello"},Bounce: true})
		assert.Equal(t, nil, err)
