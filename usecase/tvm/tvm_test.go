...
func TestTvm(t *testing.T) {
	...
	t.Run("TestExecuteMessage", func(t *testing.T) {
		...
		fileTvc, err := os.Open("../samples/Subscription.tvc")
		assert.Equal(t, nil, err)
		...
		assert.Equal(t, nil, err)
		walletAddress := "0:2222222222222222222222222222222222222222222222222222222222222222"

		// # Deploy message
		deploySet := domain.DeploySet{Tvc: tvc}
		callSet := domain.CallSet{FunctionName: "constructor", Input: json.RawMessage(`{"wallet":"` + walletAddress + `"}`)}
		signerKey := domain.NewSigner(domain.SignerKeys{keypair})

		abiUC := abi.NewAbi(tvmUC.config, tvmUC.client)

		// # Get account deploy message
		deployMessage, err := abiUC.EncodeMessage(&domain.ParamsOfEncodeMessage{Abi: abiValue, Signer: signerKey, DeploySet: &deploySet, CallSet: &callSet})
		assert.Equal(t, nil, err)
		...
		resS := &resData{}
		err = json.Unmarshal(valueNet.Result, &resS)
		assert.Equal(t, nil, err)

		bocUC := boc.NewBoc(tvmUC.config, tvmUC.client)
		parsed, err := bocUC.ParseAccount(&domain.ParamsOfParse{Boc: resS.Boc})
		assert.Equal(t, nil, err)
		...
		resB := &resBalance{}
		err = json.Unmarshal(parsed.Parsed, &resB)
		assert.Equal(t, nil, err)
		...
		result, err := tvmUC.RunExecutor(&domain.ParamsOfRunExecutor{
			Message: encodeMessage.Message,
			Account: domain.AccountForExecutor{
				domain.AccountForExecutorAccount{
					resS.Boc,
					util.BoolToPointerBool(true)}},
			Abi:                  abiValue,
			ReturnUpdatedAccount: util.BoolToPointerBool(true)})
		assert.Equal(t, nil, err)
		...
		resB2 := &resBalance{}
		err = json.Unmarshal(parsed.Parsed, resB2)
		assert.Equal(t, nil, err)

		bal1 := new(big.Int)
		bal1, _ = bal1.SetString(resB.Balance[2:], 16)

		bal2 := new(big.Int)
		bal2, _ = bal2.SetString(resB2.Balance[2:], 16)

		assert.Equal(t, -1, bal1.Cmp(bal2))
		...
		resTransS := &resTrans{}
		err = json.Unmarshal(result.Transaction, &resTransS)
		assert.Equal(t, nil, err)
		assert.Equal(t, encodeMessage.MessageID, resTransS.InMsg)
	...
	t.Run("TestRunExecutorAccNone", func(t *testing.T) {
		runParams := &domain.ParamsOfRunExecutor{
			...
			Account:              domain.AccountForExecutor{ValueEnumType: domain.AccountForExecutorNone{}},
			SkipTransactionCheck: util.BoolToPointerBool(true),
			ReturnUpdatedAccount: util.BoolToPointerBool(true),
		}

		result, err := tvmUC.RunExecutor(runParams)
		assert.Equal(t, nil, err)
	...
	t.Run("TestRunExecutorAccUninit", func(t *testing.T) {
		...
		assert.Equal(t, nil, err)
		result, err := tvmUC.RunExecutor(&domain.ParamsOfRunExecutor{Message: deployMessage.Message, Account: domain.AccountForExecutor{domain.AccountForExecutorUninit{}}, ReturnUpdatedAccount: util.BoolToPointerBool(true)})
		assert.Equal(t, nil, err)
	...
	t.Run("TestCache", func(t *testing.T) {
		fileBocText, err := os.Open("../samples/boc")
		assert.Equal(t, nil, err)
		byteBocText, err := ioutil.ReadAll(fileBocText)
		assert.Equal(t, nil, err)

		fileAbi, err := os.Open("../samples/boc.abi.json")
		assert.Equal(t, nil, err)
		abiByte, err := ioutil.ReadAll(fileAbi)
		assert.Equal(t, nil, err)

		eventsAbi := &domain.AbiContract{}
		err = json.Unmarshal(abiByte, &eventsAbi)
		assert.Equal(t, nil, err)
		...
		abiUC := abi.NewAbi(tvmUC.config, tvmUC.client)
		params := &domain.ParamsOfEncodeMessage{
			Abi:     abiValue,
			Signer:  domain.NewSigner(domain.SignerNone{}),
			Address: address,
			CallSet: &domain.CallSet{
				FunctionName: "listContenders",
			}}
		message, err := abiUC.EncodeMessage(params)
		assert.Equal(t, nil, err)

		paramsRvm := &domain.ParamsOfRunTvm{
			Message:              message.Message,
			Account:              string(byteBocText),
			Abi:                  abiValue,
			BocCache:             &domain.BocCacheType{ValueEnumType: domain.BocCacheTypeUnpinned{}},
			ReturnUpdatedAccount: util.BoolToPointerBool(true),
		}
		result, err := tvmUC.RunTvm(paramsRvm)
		assert.Equal(t, nil, err)
		...
		res := &resData{}
		err = json.Unmarshal(result.Decoded.Output, res)
		assert.Equal(t, nil, err)
		fn := "getInfoFor"
		for _, val := range res.IDS {
			message, err = abiUC.EncodeMessage(&domain.ParamsOfEncodeMessage{
				Abi:     abiValue,
				Signer:  domain.NewSigner(domain.SignerNone{}),
				Address: address,
				CallSet: &domain.CallSet{
					FunctionName: fn,
					Input:        json.RawMessage(`{"id":` + val + `}`),
				},
			})
			assert.Equal(t, nil, err)
			_, err = tvmUC.RunTvm(&domain.ParamsOfRunTvm{
				Message:  message.Message,
				Account:  string(byteBocText),
				Abi:      abiValue,
				BocCache: &domain.BocCacheType{ValueEnumType: domain.BocCacheTypeUnpinned{}},
