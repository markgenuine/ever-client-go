...
func TestTvm(t *testing.T) {
	...
	t.Run("TestExecuteMessage", func(t *testing.T) {
		...
		signerKey := domain.NewSignerKeys()
		signerKey.Keys = *keypair
	...
	config.Network.ServerAddress = domain.Chains()[1]
	clientMain, err = client.NewClientGateway(config)
	assert.Equal(t, nil, err)
	...
	t.Run("TestRunGet", func(t *testing.T) {
		...
		stateInit := domain.NewStateInitSourceStateInit()
		stateInit.Code = electorСode
		stateInit.Data = electorData

		abiUC := abi.NewAbi(tvmUC.config, tvmUC.client)
		electorAccount, err := abiUC.EncodeAccount(domain.ParamsOfEncodeAccount{StateInit: stateInit})
		assert.Equal(t, nil, err)
