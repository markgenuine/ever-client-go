...
func TestUtils(t *testing.T) {
	...
	t.Run("TestConvertAddress", func(t *testing.T) {
		valueConv1, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: AccountID, OutputFormat: domain.NewAddressStringFormatHex()})
		assert.Equal(t, nil, err)
		assert.Equal(t, HexWorkchain0, valueConv1.Address)

		valueConv2, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: valueConv1.Address, OutputFormat: domain.NewAddressStringFormatAccountID()})
		assert.Equal(t, nil, err)
		assert.Equal(t, AccountID, valueConv2.Address)

		valueConv3, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: Hex, OutputFormat: domain.NewAddressStringFormatBase64()})
		assert.Equal(t, nil, err)
		...
		valueConv4, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: Hex, OutputFormat: structAddressFormat})
		assert.Equal(t, nil, err)
		assert.Equal(t, Base64url, valueConv4.Address)

		valueConv5, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: Base64url, OutputFormat: domain.NewAddressStringFormatHex()})
		assert.Equal(t, nil, err)
		assert.Equal(t, Hex, valueConv5.Address)

		valueConv6, err := utilsUC.ConvertAddress(&domain.ParamsOfConvertAddress{Address: "-1:00", OutputFormat: domain.NewAddressStringFormatHex()})
