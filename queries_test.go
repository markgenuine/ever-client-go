...
func TestQueriesMethod(t *testing.T) {
	...
	t.Run("TestWaitFor", func(t *testing.T) {
		...
		t.Run("TestSubscribeAndEvent", func(t *testing.T) {
			...
			//get next
			for i := 0; i < 10; i++ {
				result, err = client.GetNext(subscr.Handle)
				if err != nil {
			...
			// unsubscribe
			_, err = client.Unsubscribe(subscr.Handle)
			if err != nil {
